...
import AutoGeneratedRulesReview from '../AutoGeneratedRulesReview/AutoGeneratedRulesReview.vue'
import PreexistingRulesReview from '../PreexistingRulesReview/PreexistingRulesReview.vue'
import NewRulesReview from '../NewRulesReview/NewRulesReview.vue'
import { applyRulesToTransactions, getUnmatchedTransactions, applyRulesWithDetails, getTransactionsForRule } from '../../utils/ruleMatcher.js'
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function addNewRule(rule) {
      ...
      // Calculate how many transactions this new rule would match
      let matchingTransactions = []
      if (allTransactions.value && allTransactions.value.length > 0) {
        matchingTransactions = getTransactionsForRule(rule, allTransactions.value)
      }
      ...
      const newRuleData = {
        ...
        transactions: matchingTransactions,
        frequency: matchingTransactions.length
      }
      
      console.log('addNewRule: Final rule data to add:', newRuleData)
    ...
    async function handleCombinedRulesCommit() {
      processing.value = true
      
      try {
        // First, save any new rules that were created during import
        const newRulesToSave = newRules.value.filter(rule => rule.isTemporary)
        console.log('handleCombinedRulesCommit: Saving new rules to backend:', newRulesToSave)
        
        for (const rule of newRulesToSave) {
          try {
            // Remove the temporary flag and isTemporary property before sending to backend
            const { isTemporary, ...ruleToSave } = rule
            const savedRule = await api.createRule(ruleToSave)
            console.log('handleCombinedRulesCommit: Saved rule:', savedRule)
            
            // Update the rule in the array with the backend response
            const ruleIndex = newRules.value.findIndex(r => r.id === rule.id)
            if (ruleIndex !== -1) {
              newRules.value[ruleIndex] = { ...savedRule, created_during_import: true }
            }
          } catch (error) {
        ...
        console.error('Error in handleCombinedRulesCommit:', error)
        alert('Error saving rules or importing transactions: ' + error.message)
      } finally {
        processing.value = false
      }
    }
