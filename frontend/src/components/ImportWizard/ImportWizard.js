  ...
  setup(props, { emit }) {
    ...
    async function handleCombinedRulesCommit() {
      ...
      try {
        ...
        // Get auto-generated rules that have been modified
        const autoRulesToSave = autoRules.value?.rules?.filter(rule => rule.hasChanges) || []
        allRulesToSave.push(...autoRulesToSave.map(rule => ({ ...rule, isNew: true })))
        
        console.log('handleCombinedRulesCommit: Saving rules to backend:', allRulesToSave)
        ...
        for (const rule of allRulesToSave) {
          try {
            if (rule.isNew) {
              ...
              console.log('handleCombinedRulesCommit: Created new rule:', savedRule)
            } else {
              // Remove flags before sending to backend
              const { hasChanges, isNew, ...ruleToSave } = rule
              await api.updateRule(rule.id, ruleToSave)
              console.log('handleCombinedRulesCommit: Updated existing rule:', rule.id)
            }
          } catch (error) {
