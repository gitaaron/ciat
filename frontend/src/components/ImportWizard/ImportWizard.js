...
export default {
  ...
  setup(props, { emit }) {
    ...
    async function handleCombinedRulesCommit() {
      ...
      try {
        // First, apply auto-generated rules if available
        if (autoRules.value && autoRules.value.rules && autoRules.value.rules.length > 0) {
          console.log('handleCombinedRulesCommit: Applying auto-generated rules')
          // Note: Auto-generated rules are applied automatically by the AutoGeneratedRulesReview component
          // when they are created, so we don't need to do anything special here
        }
        
        // Then, save any new rules that were created during import
