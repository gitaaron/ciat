...
if (options.minFrequency < 1) {
  console.error('❌ Error: min-frequency must be at least 1');
  process.exit(1);
}
...
async function main() {
  ...
  try {
    console.log(\`🔍 Analyzing transaction file: \${filePath}\`);
    ...
    if (!isSupportedFormat(detectedFormat)) {
      console.error(\`❌ Error: Unsupported file format: \${detectedFormat}. Supported formats: CSV, QFX\`);
      process.exit(1);
    }
    
    console.log(\`📄 File format detected: \${detectedFormat.toUpperCase()}\`);
    ...
    console.log(\`✅ Generated \${result.rules.length} auto rules\`);
